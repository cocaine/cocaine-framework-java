...
JNIEXPORT jlong JNICALL Java_ru_yandex_cocaine_dealer_Client_sendMessage
  (JNIEnv *env, jobject self, jlong dealer_ptr, jstring service, jstring handle, jstring text,
		  jboolean send_to_all_hosts, jboolean urgent, jdouble timeout, jdouble deadline, jint max_retries) {

	dealer_t *dealer = (dealer_t*) dealer_ptr;
	struct message_policy_t policy(send_to_all_hosts, urgent,0.0, timeout, deadline, max_retries);
